...
**An end-to-end healthcare analytics platform** — from a 55,500-row synthetic
admissions dataset to executive dashboards, a rigorously-validated predictive
model, and a real-world statistical reality check.
...
**The point isn't the dataset — it's the framework**:

```
Healthcare Dataset → Data Cleaning → Feature Engineering → Analytical Data Model
  → Business Analytics → 8 Analytical Modules → Intelligence Layer
  → Executive Dashboard → Business Recommendations
```

i.e. **Data → Insight → Prediction → Decision**, not `Data → Chart → Model Accuracy`.

> **New here?** [`DOCUMENTATION.md`](DOCUMENTATION.md) is the master reference —
> every stage explained simultaneously for a non-technical stakeholder, a
> Business Analyst, a Data Analyst, a Data Scientist, and a technical
> interviewer, plus 30 interview Q&As and summaries from 30 seconds to 5
> minutes. This README is the quick-start and file index.

## At a Glance

| | |
|---|---|
| **Records processed** | 55,500 raw → 54,860 cleaned (98.85% retention) |
| **Analytics modules** | 8 (Executive, Hospital, Financial, Insurance, Doctor, Patient, Resource Planning, Predictive) |
| **ML models compared** | 9 families, full ablation + hyperparameter tuning |
| **Best test accuracy** | 43.2% (Random Forest) vs. 33.3% baseline — statistically significant, McNemar p = 8.7e-08 |
| **External real dataset** | 692 World Bank observations, 6 countries, 2000–2024 |
| **Total billing analyzed** | $1.40B |
| **Dashboards** | 2 interactive HTML artifacts, 11 tabs / 12 pages combined |

## Live Dashboards

| Deliverable | What it is |
|---|---|
| **[Healthcare Intelligence Console](dashboard/console.html)** | 11-tab interactive dashboard (Tableau substitute) — every module's charts and tables, the full ML model comparison, a Reality Check tab, and a Global Context tab built from a real downloaded World Bank dataset. |
...
Open either `.html` file directly in a browser — both load their paired
`.css`/`.js` file from the same folder and are otherwise self-contained, no
server or build step required. Want a real Tableau workbook instead? See
[`reports/08_tableau_build_guide.md`](reports/08_tableau_build_guide.md) —
the star schema in `data/model/` was built specifically to connect directly.
...
data/
  ...
  model/advanced_ml/best_model.joblib       the tuned final model (~348 MB, Git LFS — see report note)
...
dashboard/
  console.html / console.css / console.js       Tableau-substitute executive console
  blueprint.html / blueprint.css / blueprint.js  Figma-substitute architecture blueprint

DOCUMENTATION.md   Master reference — every audience, every stage, interview prep
README.md          This file
```
...
**Step 7 requires internet access** — it calls the live World Bank API and
will produce slightly different numbers on re-run as the World Bank revises
recent-year estimates (expected behavior for a live external source, not a
bug). **Step 8 is the slow one** — feature ablation, a 9-model comparison,
and `RandomizedSearchCV` tuning on two model families takes roughly 20
minutes on ~44K rows; it also saves a ~348 MB model file (Git LFS-tracked —
see the report's practical note). Step 9 finishes in seconds.
...
[`reports/06_advanced_ml_pipeline.md`](reports/06_advanced_ml_pipeline.md) runs the genuine
attempt this question deserves: independence testing, 6 feature-set
...
Beyond the standard EDA, [`reports/07_deep_eda_profiling.md`](reports/07_deep_eda_profiling.md) runs a
...
To ground the platform against something outside itself, [`reports/04_reality_check.md`](reports/04_reality_check.md)
...
Beyond individually-cited figures, [`scripts/07_external_worldbank_data.py`](scripts/07_external_worldbank_data.py)
...
(including a full 23-year Japan time series). Full detail:
[`reports/05_external_data_sources.md`](reports/05_external_data_sources.md).
...
| Visualization / BI | HTML/CSS/JS interactive dashboards (Tableau-ready star schema included) |
| External Data | World Bank Open Data API (`requests`) |
...
See [`dashboard/blueprint.html`](dashboard/blueprint.html) → *Page 10, Future Scope* for what real deployment would need to add, *Page 11, Reality Check* for the full benchmark comparison, and *Page 12, Global Context* for the World Bank comparison — or [`DOCUMENTATION.md`](DOCUMENTATION.md) §21 for the fully-explained version.

---

<div align="center">

© 2026 Sneha Juyal — see [`LICENSE`](LICENSE)
